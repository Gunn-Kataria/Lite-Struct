...
* `palette.light` / `palette.dark` (bg, surface, border, text*, **primary = orange**, `gradient`, danger/success/warning + soft variants, navigation-chrome colours),
...
* `Button` — variants `primary` (orange gradient + glow, lifts on hover, squishes on press), `secondary`, `ghost`, `danger`; `icon` takes a lucide component; `testID` → `data-testid`.
